...
def plot_confusion_matrix(cm, normalize=False, title='Matriz de Confusão', cmap=None):
    ...
    if normalize:
        ...
        logging.info("Matriz de Confusão normalizada")
    else:
        logging.info("Matriz de Confusão não normalizada")
...
def show_and_export_results(synthetic_accuracies, synthetic_precisions, synthetic_recalls, synthetic_f1_scores,
    ...
    create_and_save_plot(classifier_type, synthetic_accuracies, synthetic_precisions, synthetic_recalls,
...
def get_adversarial_instance(latent_dim, output_data_shape, activation_function, initializer_mean,
...
def run_experiment(dataset, number_samples_true, number_samples_false, output_shape, k,
                   ...
                   dense_layer_sizes_d=None, dataset_type=None, output_label=None, initializer_mean=None,
                   initializer_deviation=None, last_layer_activation=DEFAULT_CONDITIONAL_LAST_ACTIVATION_LAYER,
    ...
    for i, (train_index, test_index) in enumerate(stratified.split(dataset.iloc[:, :-1], dataset.iloc[:, -1])):
        adversarial_model = get_adversarial_instance(latent_dim, output_shape, activation_function, initializer_mean,
                                                     initializer_deviation, dropout_decay_rate_g, dropout_decay_rate_d,
        ...
        instance_classifier = get_classifier(classifier_type, x_training, y_training, output_shape, dataset_type)
...
if __name__ == "__main__":
    ...
    run_experiment(dataset_file, arguments.num_samples_class_malware, arguments.num_samples_class_benign, output_shape,
                   arguments.k_fold,
